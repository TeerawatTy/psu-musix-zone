...
const RoomPageClient = ({ sessionData, reservations, errorMessage }: { sessionData: any, reservations: any, errorMessage: string }) => {
  return (
    <div
      className="p-4 sm:p-8 max-w-full mx-auto grid grid-cols-1 sm:grid-cols-2 gap-8"
      style={{
    ...
    >
      ...
      <div className="bg-white bg-opacity-50 p-6 sm:p-10 rounded-lg shadow-lg">
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-6 sm:mb-10 text-center">UPCOMING RESERVATIONS</h2>
      ...
      <div className="p-6 sm:p-10 rounded-lg shadow-lg w-full"
        style={{
      ...
      >
        <h1 className="text-2xl sm:text-3xl font-bold text-white mt-6 text-center">PRACTICE ROOM RESERVATIONS</h1>

        {sessionData ? (
          <div>
            <h2 className="text-xl sm:text-2xl text-orange-500 mt-2 mb-6 sm:mb-10 text-center">Welcome!, {sessionData?.name}</h2>
            <ReserveRoomForm />
          </div>
        ) : (
          <div>
            <p className="text-red-500 my-4 text-center text-lg sm:text-xl">
              Session cookie not found. Please log in to make a reservation.
            </p>
            <Link href="/login" className="flex items-center justify-center mt-6 sm:mt-10">
              <span className="text-white text-lg sm:text-2xl font-bold px-6 py-2 border-2 border-white rounded-md hover:bg-orange-500 hover:text-white hover:border-white transition duration-200">
                Login
              </span>
            </Link>
          </div>
